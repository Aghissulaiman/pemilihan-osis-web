...
export default function ProfilePage() {
  ...
  // Left column: NISN & Email | Right column: Kelas & No. HP
  const leftItems = [
    { icon: IdCard, label: "NISN",   value: user.nisn,  desc: "Nomor Induk Siswa",      delay: "0.45s" },
    { icon: Mail,   label: "Email",  value: user.email, desc: "Alamat Surat Elektronik", delay: "0.5s"  },
  ];
  const rightItems = [
    { icon: School, label: "Kelas",  value: user.kelas, desc: "Kelas Aktif",   delay: "0.45s" },
    { icon: Phone,  label: "No. HP", value: user.phone, desc: "Nomor Handphone", delay: "0.5s"  },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#eef3fb] overflow-x-hidden relative" style={{ fontFamily: "'Sora',sans-serif" }}>
      ...
      <style>{`
        ...
        .anim-verified{animation:fadeUp .5s .55s both}
      ...
      <div className="relative z-10 px-5 pt-10 pb-[100px] overflow-hidden" style={{ background: "linear-gradient(150deg,#1e3a8a 0%,#1d4ed8 40%,#2563eb 70%,#3b82f6 100%)" }}>
        ...
        <div className="relative z-30 max-w-[480px] mx-auto flex flex-col items-center">
          ...
          <div className="anim-u4 flex gap-[10px]">
            {[{ val: "XII", lbl: "Tingkat" }, { val: "RPL", lbl: "Jurusan" }, { val: null, lbl: "Aktif", star: true }].map(({ val, lbl, star }) => (
      ...
      <div className="relative z-20 max-w-[500px] mx-auto px-4 pb-10">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex flex-col gap-3">
            {leftItems.map(({ icon: Icon, label, value, desc, delay }) => (
              <div
                key={label}
                className="card-hover bg-white/90 backdrop-blur-xl border border-white/90 rounded-[18px] p-4 flex flex-col gap-3 cursor-default transition-all duration-[250ms]"
                style={{ boxShadow: "0 2px 12px rgba(37,99,235,.06),0 1px 0 rgba(255,255,255,.8) inset", animation: `fadeUp .5s ${delay} both` }}
              >
                <div className="ic-shimmer relative w-[42px] h-[42px] rounded-[12px] flex items-center justify-center flex-shrink-0 overflow-hidden" style={{ background: "linear-gradient(135deg,#dbeafe,#bfdbfe)", boxShadow: "0 2px 8px rgba(37,99,235,.12),0 1px 0 rgba(255,255,255,.7) inset" }}>
                  <Icon size={19} color="#2563eb" strokeWidth={1.8} />
                </div>
                <div className="min-w-0">
                  <div className="text-[9px] font-bold tracking-[1.5px] uppercase text-slate-400 mb-[3px]">{label}</div>
                  <div className="text-[12px] font-semibold text-[#1e3a5f] tracking-tight truncate">{value}</div>
                  <div className="text-[9.5px] text-slate-400 mt-[2px]">{desc}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Right col */}
          <div className="flex flex-col gap-3">
            {rightItems.map(({ icon: Icon, label, value, desc, delay }) => (
              <div
                key={label}
                className="card-hover bg-white/90 backdrop-blur-xl border border-white/90 rounded-[18px] p-4 flex flex-col gap-3 cursor-default transition-all duration-[250ms]"
                style={{ boxShadow: "0 2px 12px rgba(37,99,235,.06),0 1px 0 rgba(255,255,255,.8) inset", animation: `fadeUp .5s ${delay} both` }}
