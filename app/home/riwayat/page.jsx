...
export default function RiwayatVotingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#eef3fb] overflow-x-hidden relative" style={{ fontFamily: "'Sora', sans-serif" }}>
      ...
      {/* Google Fonts */}
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;500;600;700&family=Fraunces:opsz,wght@9..144,700&display=swap');
      ...
      <nav className="sticky top-0 z-50 h-[60px] flex items-center justify-between px-5 bg-white/75 backdrop-blur-2xl border-b border-blue-100/60">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-[15px] text-[#1e3a5f] tracking-tight">Riwayat Voting</span>
        </div>
      ...
      >
        ...
        />
        {/* Orbs */}
        <div className="absolute w-[500px] h-[500px] rounded-full -top-44 -right-36 pointer-events-none" style={{ background: "radial-gradient(circle,rgba(255,255,255,0.1) 0%,transparent 65%)" }} />
        <div className="absolute w-[300px] h-[300px] rounded-full bottom-8 -left-24 pointer-events-none" style={{ background: "radial-gradient(circle,rgba(255,255,255,0.07) 0%,transparent 65%)" }} />
        {/* Wave cutout */}
        <div className="absolute bottom-0 left-0 right-0 h-14 bg-[#eef3fb] z-20" style={{ clipPath: "ellipse(60% 100% at 50% 100%)" }} />

        <div className="relative z-30 max-w-md mx-auto flex flex-col items-center">
          {/* Icon shell with spin ring */}
          <div
            className="anim-icon relative w-[90px] h-[90px] rounded-full p-[3px] mb-[22px]"
            style={{ background: "linear-gradient(135deg,rgba(255,255,255,0.45),rgba(255,255,255,0.1))", boxShadow: "0 12px 40px rgba(0,0,0,0.2),0 0 0 6px rgba(255,255,255,0.07)" }}
          >
            {/* spinning conic ring */}
            <div
              className="spin-ring absolute rounded-full pointer-events-none"
              style={{ inset: "-6px", background: "conic-gradient(from 0deg,transparent,rgba(255,255,255,0.3) 60deg,transparent 120deg)" }}
            />
            <div className="relative w-full h-full rounded-full bg-white/15 backdrop-blur-md flex items-center justify-center">
              <CheckCircle size={36} color="rgba(255,255,255,0.9)" strokeWidth={1.5} />
            </div>
          </div>

          <p className="anim-1 text-[9px] font-bold tracking-[3.5px] uppercase text-white/45 mb-2">Rekap Suara</p>
          <h2 className="anim-2 text-[30px] font-bold text-white tracking-tight mb-2" style={{ fontFamily: "'Fraunces',serif", textShadow: "0 2px 16px rgba(0,0,0,0.2)" }}>
            Riwayat Voting Kamu
          </h2>
          <p className="anim-3 text-[12.5px] text-white/60">Berikut adalah rekap suara yang telah kamu berikan</p>
        </div>
      </div>
      ...
      <div className="relative z-20 max-w-[500px] mx-auto px-4 pt-2 pb-10">
        ...
        <div className="anim-sec flex items-center gap-2 mb-3.5 mt-2">
          <span className="text-[9px] font-bold tracking-[2.5px] uppercase text-slate-400">Daftar Suara</span>
        ...
        {riwayat.length > 0 ? (
          <div className="flex flex-col gap-3">
            {riwayat.map((item) => (
              <div
                ...
                style={{ boxShadow: "0 4px 24px rgba(37,99,235,0.08),0 1px 0 rgba(255,255,255,0.8) inset" }}
              >
                {/* Accent bar */}
                ...
                <div className="p-[22px]">
                  ...
                  <div className="flex items-center gap-4">
                    <div
                      ...
                      style={{ fontFamily: "'Fraunces',serif", boxShadow: "0 4px 12px rgba(37,99,235,0.15),0 1px 0 rgba(255,255,255,0.8) inset" }}
                    >
                      {item.kandidat.charAt(0)}
                    </div>
                    <div>
                      <div className="text-[9px] font-bold tracking-[1.5px] uppercase text-slate-400 mb-1">Kandidat Dipilih</div>
                      <div className="text-[16px] font-bold text-[#1e3a5f] tracking-tight mb-1" style={{ fontFamily: "'Fraunces',serif" }}>
                        {item.kandidat}
                      </div>
                  ...
                  {/* Divider */}
                  <div className="my-[18px] h-px" style={{ background: "linear-gradient(90deg,transparent,#e2e8f0,transparent)" }} />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-[9px] font-bold tracking-[1.5px] uppercase text-slate-400 mb-0.5">Tanggal Voting</div>
                      <div className="text-[13px] font-semibold text-[#1e3a5f]">{item.tanggal}</div>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div
            className="anim-card bg-white/90 backdrop-blur-xl border border-dashed border-blue-200 rounded-[22px] px-6 py-14 text-center"
          >
            <div className="w-[72px] h-[72px] rounded-full bg-gradient-to-br from-blue-100 to-blue-50 flex items-center justify-center mx-auto mb-[18px]" style={{ boxShadow: "0 4px 16px rgba(37,99,235,0.1)" }}>
              <CheckCircle size={32} color="#93c5fd" strokeWidth={1.5} />
            </div>
            <h3 className="text-[20px] font-bold text-[#1e3a5f] mb-2" style={{ fontFamily: "'Fraunces',serif" }}>
              Belum Ada Riwayat
            </h3>
            <p className="text-[13px] text-slate-400 leading-relaxed">
              Kamu belum memberikan suara.<br />Yuk gunakan hak pilihmu!
            </p>
          </div>
        )}
        ...
        <div className="anim-cta mt-4">
          <Link
            ...
            style={{ background: "linear-gradient(135deg,#2563eb,#1d4ed8)", boxShadow: "0 6px 24px rgba(37,99,235,0.35),0 1px 0 rgba(255,255,255,0.15) inset" }}
          >
            <ArrowLeft size={17} />
            Kembali ke Beranda
          </Link>
