...
export default function KandidatManager() {
    ...
    // Cek apakah no_kandidat sudah ada (kecuali untuk kandidat yang sedang diedit)
    const isNoKandidatExist = (noKandidat, currentId = null) => {
        return kandidat.some(k => 
            k.no_kandidat === parseInt(noKandidat) && 
            (currentId ? k.id !== currentId : true)
        )
    }
    ...
    const handleSubmit = async (e) => {
        ...
        // Validasi no_kandidat tidak boleh kosong
        if (!form.no_kandidat) {
            setModalError("Nomor kandidat wajib diisi")
            setModalLoading(false)
            return
        }

        // Validasi no_kandidat harus angka positif
        if (form.no_kandidat < 1) {
            setModalError("Nomor kandidat harus lebih dari 0")
            setModalLoading(false)
            return
        }

        // Validasi no_kandidat unik (kecuali untuk edit data yang sama)
        const noKandidatNumber = Number(form.no_kandidat)
        const isDuplicate = isNoKandidatExist(noKandidatNumber, editData?.id)
        
        if (isDuplicate) {
            setModalError(`Nomor kandidat ${noKandidatNumber} sudah digunakan. Silakan pilih nomor lain.`)
            setModalLoading(false)
            return
        }

        const payload = {
            no_kandidat: noKandidatNumber,
            nama: form.nama.trim(),
        ...
        if (error) {
            // Handle error unique constraint dari database
            if (error.code === '23505' && error.message.includes('no_kandidat')) {
                setModalError(`Nomor kandidat ${form.no_kandidat} sudah digunakan di database. Silakan pilih nomor lain.`)
            } else {
    ...
    const handleHapus = async (id) => {
        ...
        const kandidatToDelete = kandidat.find(k => k.id === id)
        if (kandidatToDelete?.vote_count > 0) {
            alert("Tidak dapat menghapus kandidat yang sudah memiliki suara")
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-lg bg-white border border-gray-100 rounded-2xl shadow-xl transition-all duration-300">
                        ...
                        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
                            ...
                            {/* No Kandidat - dengan validasi unique */}
                            <div>
                                <label className="block text-[11px] uppercase tracking-wider text-gray-400 font-semibold mb-1.5">
                                    No. Kandidat <span className="text-red-400">*</span>
                                    <span className="text-gray-300 text-[9px] ml-2">(harus unik)</span>
                                </label>
                                <input
                                    type="number"
                                    name="no_kandidat"
                                ...
                                />
                                {/* Tampilkan pesan jika no_kandidat sudah ada (real-time) */}
                                {form.no_kandidat && !editData && isNoKandidatExist(form.no_kandidat) && (
                                    <p className="text-red-500 text-xs mt-1">
                                        ⚠️ Nomor {form.no_kandidat} sudah digunakan
                                    </p>
                                )}
                                {form.no_kandidat && editData && 
                                 isNoKandidatExist(form.no_kandidat, editData.id) && (
                                    <p className="text-red-500 text-xs mt-1">
                                        ⚠️ Nomor {form.no_kandidat} sudah digunakan oleh kandidat lain
                                    </p>
                                )}
                            </div>
                            ...
                            <div>
                                <label className="block text-[11px] uppercase tracking-wider text-gray-400 font-semibold mb-1.5">
                                    Visi <span className="text-red-400">*</span>
                                </label>
                                <input
                                    type="text"
                                    name="visi"
                            ...
                            <div>
                                <label className="block text-[11px] uppercase tracking-wider text-gray-400 font-semibold mb-1.5">
                                    Misi <span className="text-red-400">*</span>
                                </label>
                                <textarea
                                    name="misi"
                                    value={form.misi}
                            ...
                            <div className="flex items-center gap-3 pt-2">
                                ...
                                <button
                                    type="submit"
                                    disabled={modalLoading || (form.no_kandidat && !editData && isNoKandidatExist(form.no_kandidat))}
                                        ...
                                        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button
                                                    ...
                                                    disabled={deletingId === k.id || k.vote_count > 0}
                                                    className={`w-9 h-9 rounded-lg bg-gray-50 hover:bg-red-50 text-gray-400 hover:text-red-500 border border-gray-100 flex items-center justify-center transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed`}
                                                    title={k.vote_count > 0 ? "Tidak bisa menghapus kandidat yang sudah memiliki suara" : "Hapus"}
                                                >
